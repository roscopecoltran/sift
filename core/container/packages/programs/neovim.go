...
func (neovim NeoVim) Build(config sift.Config) error {
	...
	makeDeps := sift.MakeCmd{Dir: buildDepsDir}.Cmd()
...
func (neovim NeoVim) Install(config sift.Config) error {
	buildDir := filepath.Join(config.BuildDir(neovim), "build")
...
func (neovim NeoVim) Dependencies() []sift.Package {
	return []sift.Package{}
}
