...
echo "Running tests..."
sift run sift make test

echo "Update version..."
grepped_version=$(grep -o "v[0-9]*\.[0-9]*\.[0-9]*" sift/version_basic.go)
old_version=${grepped_version:1}
sed -i.bak 's/fmt\.Println("v'$old_version'")/fmt.Println("v'$version'")/' sift/version_basic.go
sed -i.bak 's/fmt\.Println("v'$old_version' (PRO)")/fmt.Println("v'$version' (PRO)")/' sift/version_pro.go
rm sift/version_basic.go.bak
rm sift/version_pro.go.bak
...
echo "Build binaries..."
sift run sift make build

echo "Update repository..."
git add sift/cli.go download.sh README.md CHANGELOG.md CONTRIBUTORS
