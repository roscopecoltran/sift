...
type ImportDefaultDefs struct {
	Languages   bool     `short:"il" long:"import-patterns-langs" description:"import the definitions to find languages related patterns"`
	Libraries  	bool     `short:"ik" long:"import-patterns-libs" description:"import the default to find libraries keywords related patterns"`
	Brands    	bool     `short:"ib" long:"import-patterns-brands" description:"import the definitions to find brands related patterns"`
	Packages    bool     `short:"ip" long:"import-patterns-packages" description:"import the definitions to lookup through 'package-managers' per operating systems (eg. Alpine, Linux) and create a mapping matrix."`
	Vendors   	bool     `short:"iv" long:"import-patterns-vendors" description:"import the default list of 'vendors' related patterns"`
	Docs      	bool     `short:"id" long:"import-patterns-docs" description:"import the default list of 'documentations' related patterns."`
}
...
type Options struct {
	...
	MatchTopics struct {
		DefaultLanguages     		map[string]map[string][]string
		DefaultOpSystems     		map[string]map[string][]string
		DefaultKeywords     		map[string]map[string][]string
		DefaultEntities     		map[string]map[string][]string
		DefaultSecurity     		map[string]map[string][]string
	} `group:"Match patterns lists" json:"-"`

	MatchReports struct {
		Status        		bool   	`short:"mrs" long:"match-reports-active" description:"Activate the extend reporting (VCS informations, issues and web related links)."`
		Mode               	string  `short:"mrm" long:"match-reports-mode" description:"matching reporting mode (summary or full contextual informations)"`
	} `group:"Match patterns lists" json:"-"`	

	KnowledgeBase struct {
		Status        		bool   `short:"kbs" long:"kb-active" description:"Activate/Load the local knowledge base."`
		LearnMode        	bool   `short:"kbl" long:"kb-learn" description:"learn from reports"`
		ReinforcedMode    	bool   `short:"kbr" long:"kb-reinforced-learn" description:"reinforce knowledge base with new patterns or topics"`
	} `group:"Knowledge base patterns analysis and queries" json:"-"`	

	TopicModeling struct {
		Status        		bool   `short:"atd" long:"activate-topics-discovery" description:"Append topics predictions to reports or knowledge base."`
		ReinforcedMode    	bool   `short:"kbr" long:"reinforced-topics" description:"reinforce topic models with new discoveries."`
	} `group:"Match topics models" json:"-"`	

	/*
	Import struct {
		Definitions 	struct {
			LoadDef 	ImportDefaultDefs
			CustomDefs  map[string][]string `long:"custom-def" description:"add new custom definitions to scan." custom-def:"PATTERN"`
		} `group:"Import Generic Patterns definitions" json:"-"`
	} `group:"Pre-load a set of tasks or definitions to match automatically." json:"-"`
	*/
...
func (o *Options) LoadDefaults() {
	...
	// var matches []string
	// for _, e := range types {
	//for m := range o.MatchPatterns {
	global.reports   	= []Report{}
	global.topics  	 	= []Topic{}
	global.insights  	= []Insight{}

	patternArgs 		:= [][]string{o.MatchTopics}

	for k, v := range m { 
	    fmt.Printf("key[%s] value[%s]\n", k, v)
	}

	for _, m := range patternArgs {
		// types = append(types, m)
		o.MatchTopics[m] 			= make(map[string]map[string][]string)
	}

	o.MatchReports.Status			= true
	o.MatchReports.Mode 			= "full"
	o.KnowledgeBase.LearnMode		= true
	o.KnowledgeBase.ReinforcedMode	= false
	o.TopicModeling.Status 			= true
	o.TopicModeling.ReinforcedMode	= false
