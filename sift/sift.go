...
import (
	...
	"github.com/roscopecoltran/sniperkit-sift/git"
...
type MatchPatterns struct {
	Patterns     []string
	ShebangRegex *regexp.Regexp
}

type MatchTopics struct {
	Topics     []string
}
...
var global = struct {
	...
	insights              []Insight
	topics                []Topic
	keywordTypesMap       map[string]MatchPatterns
	topicTypesMap         map[string]MatchTopics
	matchTopics 		  []string
	...
	gitignoreCache        *git.GitIgnoreCache
...
func processDirectory(dirname string) {
	...
	var gic *git.Checker
	if options.Git {
		gic = git.NewCheckerWithCache(global.gitignoreCache)
	...
	nextEntry:
		for _, fi := range entries {
			...
			if fi.IsDir() {
				...
				if options.Git {
					if fi.Name() == git.GitFoldername || gic.Check(fullpath, fi) {
						continue nextEntry
					}
				}
			...
			if options.Git {
				if fi.Name() == git.GitIgnoreFilename || gic.Check(fullpath, fi) {
					continue
				}
			}
...
func executeSearch(targets []string) (ret int, err error) {
	...
	global.gitignoreCache = git.NewGitIgnoreCache()
